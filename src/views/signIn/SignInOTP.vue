...
import {SignInOTPModel} from "~/store/forms/loginForms";
...
export default defineComponent({
  ...
  setup(props){
    const pwdLength = 6;
    const model = new SignInOTPModel ({
      onCancel(){
        model.resetState();
      }
    });
    ...
    const onSubmitOTP = ()=> otpMixin.onSubmit({
      routeName: RouterNames.SignIn_UpdatePwd
    });

    onMounted(async ()=>{
      await otpMixin.mount(()=>facade.apiService.forgotPassword({
